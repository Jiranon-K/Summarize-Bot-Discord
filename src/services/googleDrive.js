// Secure Credentials Management Version
...
class GoogleDriveService {
  ...
  async initialize() {
    ...
    try {
      console.log(
        "🔐 Initializing Google Drive service with secure credentials..."
      );
  ...
  async loadCredentials() {
    ...
    for (const loadMethod of credentialsSources) {
      try {
        ...
        console.warn(
          `⚠️ Failed to load credentials from ${loadMethod.name}:`,
          error.message
        );
        continue;
      }
      ...
      try {
        ...
        if (secretExists) {
          ...
          try {
            ...
            console.log(
              "✅ Using Docker secret (JSON format) from:",
              secretPath
            );
            ...
            try {
              ...
              console.log(
                "✅ Using Docker secret (Base64 format) from:",
                secretPath
              );
              this.logAuditEvent(
                "credentials_loaded",
                "Docker Secrets (Base64)"
              );
              return credentials;
            } catch (base64Error) {
              console.warn(
                `⚠️ Invalid secret format in ${secretPath}:`,
                base64Error.message
              );
            }
          }
        ...
        console.warn(
          `⚠️ Cannot read Docker secret from ${secretPath}:`,
          error.message
        );
      }
    }
  ...
  async loadFromEnvironmentVariables() {
    ...
    try {
      ...
      throw new Error(
        `Invalid individual environment variables: ${error.message}`
      );
    }
  }
  ...
  async loadFromCloudSecretManager() {
    ...
    try {
      ...
      console.warn(
        "⚠️ Failed to load from Google Secret Manager:",
        error.message
      );
      return null;
    }
  ...
  async loadFromVault() {
    ...
    try {
      ...
      const secretPath =
        process.env.VAULT_SECRET_PATH || "secret/data/google-credentials";
  ...
  async listFiles(folderId = null) {
    ...
    try {
      ...
      if (!targetFolderId) {
        throw new Error(
          "Google Drive Folder ID ไม่ได้ระบุใน environment variables"
        );
      }
      const query = `'${targetFolderId}' in parents and trashed = false and (
...
            )`;
  ...
  async getFile(fileId) {
    ...
    try {
      ...
      this.logAuditEvent(
        "file_access_failed",
        `Failed to access file: ${fileId}, Error: ${error.message}`
      );
      if (error.code === 404) {
  ...
  getCredentialsSourceInfo() {
    ...
    if (require("fs").existsSync("/run/secrets/google_credentials"))
  ...
  clearCredentials() {
    ...
    this.logAuditEvent(
      "credentials_cleared",
      "Credentials cleared from memory for security"
    );
  }
